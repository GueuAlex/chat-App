...
if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password) && !empty($level)){
            ...
            if(isset($_FILES['image'])){ //if file is uploaded
                ...
                if(in_array($img_ext, $extention) === true){// if user upload img extention is correct
                    ...
                    if(move_uploaded_file($tmp_name, "images/".$new_img_name)){//if user upload img to our folder sucessfully
                        $status = "Hors ligne";//once user signed up then status will active now
                        $random_id = rand(time(),10000000);//creating random id for user

                        //isertion des données dans la users de notre BDD
                        $sql2 = mysqli_query($conn, "INSERT INTO users(unique_id, fname, lname, email, password, img, post, status) VALUES({$random_id}, '{$fname}','{$lname}', '{$email}', '{$password}', '{$new_img_name}', '{$level}', '{$status}')");
